...
export const Route = createFileRoute("/image-tools/invert")({
  head: () => toolHead("image-tools", "invert"),
  component: Page,
});
...
function Page() {
  ...
  const download = async () => {
    ...
    try {
      ...
      downloadBlob(blob, file.name.replace(/\.\w+$/, "") + "-invert." + (isPng ? "png" : "jpg"));
    } catch (e) { setError(e instanceof Error ? e.message : "Could not process image."); }
  };

  return (
    <ToolShell categorySlug="image-tools" toolSlug="invert"
      intro="Invert the colors of any image for a photographic negative effect."
