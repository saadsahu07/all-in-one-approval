...
export const Route = createFileRoute("/image-tools/contrast")({
  head: () => toolHead("image-tools", "contrast"),
  component: Page,
});
...
function Page() {
  ...
  const [v, setV] = useState(130);
  const filter = `contrast(${v}%)`;
  ...
  const download = async () => {
    ...
    downloadBlob(blob, file.name.replace(/\.\w+$/, "") + "-contrast." + (isPng ? "png" : "jpg"));
  };
  return (
    <ToolShell categorySlug="image-tools" toolSlug="contrast"
      intro="Boost or reduce contrast on any image with live preview."
      ...
      {file && preview && (
        <div className="mt-4 space-y-4">
          ...
          <div className="max-w-sm"><Label>Contrast: {v}%</Label><input type="range" min={0} max={300} value={v} onChange={(e) => setV(+e.target.value)} className="w-full accent-[var(--accent)]" /></div>
